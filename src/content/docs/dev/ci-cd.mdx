...
description: The `CI` workflow codifies our enterprise readiness guardrails so
  every commit is vetted for quality, security, and operational excellence. The
...
implemented in [`.github/workflows/ci.yml`](https://github.com/apotheon-ai/apotheon.ai/blob/main/.github/workflows/ci.yml) and
...
3. **Core quality gates.** Each job runs `npm ci`, `npm run lint`,
   ...
   follow the [testing workflow](/docs/dev/testing/) so CI and local executions stay
   aligned.
...
strict budgets defined in [`lighthouserc.json`](https://github.com/apotheon-ai/apotheon.ai/blob/main/lighthouserc.json). Key
...
restricted via [`zap-baseline.conf`](https://github.com/apotheon-ai/apotheon.ai/blob/main/zap-baseline.conf) so only the static
...
our curated configuration ([`.gitleaks.toml`](https://github.com/apotheon-ai/apotheon.ai/blob/main/.gitleaks.toml)). The config
...
Refer to [docs/dev/TESTING.md](/docs/dev/testing/) for Playwright snapshot refreshes
