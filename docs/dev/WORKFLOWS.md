...
- **Content collections:** Long-form “about” pages stay under `src/content/marketing/about`, while industry narratives moved into the dedicated `src/content/industries` collection (see [`docs/dev/INDUSTRIES.md`](./INDUSTRIES.md)). Structured solution content remains in `src/content/solutions/` (documented in [`docs/dev/SOLUTIONS.md`](./SOLUTIONS.md)).
- **Industry automation:** The industries schema captures hero metadata (with icon slugs), sector pressures, compliance highlights, validated solution references, persona-focused use cases, and dual CTAs. Astro refuses to build when a referenced solution slug does not exist, protecting cross-links automatically.
- **Author workflow:** Drop or edit an MDX file inside `src/content/industries`, run `npm run typecheck` to ensure schema compliance, and execute `npm run build` so static routes regenerate. Index pages (`/industries`, `/solutions`, `/about`) automatically enumerate published entries; manual routing is never required.
- **Reusable components:** Shared layout primitives (hero shell, CTA rows, industry-specific components) live in `src/components/marketing/` and `src/components/industries/`. Follow inline comments covering SEO, accessibility, and hydration guidance before extending any template.
- **Homepage industries preview contract:** `src/components/homepage/IndustriesPreview.astro` now queries the `industries` collection directly. After updating ordering or copy, run `npm run build` to refresh the static export and Pagefind indexes, then execute the release gate (`npm run lint`, `npm run typecheck`, `npm run test`, `npm run test:e2e`, `npm run build`, `npm run ladle:build`).
...
- **Solutions payload audit:** Anytime you touch solution content or shared styles/scripts, execute `node ./scripts/ci/solutions-payloads.mjs` after `npm run build`. The helper summarizes first-party CSS/JS weights per route so you can validate the 50 KB budget locally before the Lighthouse CI gate runs.
